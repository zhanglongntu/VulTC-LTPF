...
def read_data_to_dataframe(filename,is_train=False):
    ...
    return data[['abstract_func_before', 'description', 'cwe_ids_tip']]
...
def convert_dataframe_to_dataset(data):
    ...
    for idx, row in data.iterrows():
        ...
        examples['label'].append(int(row['cwe_ids_tip']))
...
for epoch in range(num_epochs):
    ...
    for idx, cwe_id in enumerate(tail_cwe_ids):
        ...
        if n_augment > 0:
            tail_examples = [ex for ex in train_dataset if int(ex['label']) == cwe_id]
    ...
    for data in augmented_examples:
        input_example = InputExample(text_a=data['text_a'], text_b=data['text_b'], label=data['label'])
        dataset['train'].append(input_example)

    for class_name, error_rate, n_augment in tail_error_info:
        logging.info(
            "Tail Class: {}, Error Rate: {:.4f}, Samples to Augment: {}".format(class_name, error_rate, n_augment))

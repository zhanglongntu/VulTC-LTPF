...
myverbalizer = ManualVerbalizer(tokenizer, classes=classes,
                                label_words={
                                    'CWE-119': ['buffer overflow', 'memory violation', 'out-of-bounds access'],
                                    'CWE-125': ['out-of-bounds read', 'information leak', 'data exposure'],
                                    'CWE-787': ['memory corruption', 'out-of-bounds write', 'invalid memory access'],
                                    'CWE-476': ['null pointer dereference', 'null access', 'dereferencing null'],
                                    'CWE-20': ['input validation', 'sanitization error', 'insecure input handling'],
                                    'CWE-416': ['use after free', 'dangling pointer', 'use of freed memory'],
                                    'CWE-190': ['integer overflow', 'arithmetic overflow', 'overflow error'],
                                    'CWE-200': ['information disclosure', 'data leak', 'sensitive data exposure'],
                                    'CWE-399': ['resource management error', 'resource handling', 'improper resource management'],
                                    'CWE-264': ['permissions and access control', 'authorization', 'privilege escalation'],
                                    'CWE-120': ['buffer overflow', 'memory corruption', 'stack buffer overflow'],
                                    'CWE-362': ['race condition', 'concurrency issue', 'data race'],
                                    'CWE-400': ['resource exhaustion', 'denial of service', 'system overload'],
                                    'CWE-401': ['memory leak', 'resource leak', 'memory allocation failure'],
                                    'CWE-189': ['integer underflow', 'arithmetic underflow', 'underflow error'],
                                    'CWE-617': ['uncontrolled resource', 'assertion failure', 'resource leak'],
                                    'CWE-835': ['infinite loop', 'unbounded loop', 'endless iteration'],
                                    'CWE-772': ['missing release of resource', 'handle leak', 'resource retention'],
                                    'CWE-287': ['authentication failure', 'login bypass', 'unauthorized access'],
                                    'CWE-369': ['divide by zero', 'arithmetic exception', 'zero division error'],
                                    'CWE-22': ['path traversal', 'file access vulnerability', 'directory traversal'],
                                    'CWE-415': ['double free', 'multiple memory release', 'double free vulnerability'],
                                    'CWE-674': ['uncontrolled recursion', 'stack overflow', 'recursive loop'],
                                    'CWE-122': ['heap-based buffer overflow', 'dynamic memory error', 'heap corruption'],
                                    'CWE-254': ['protection mechanism failure', 'security mechanism bypass', 'insecure protection'],
                                    'CWE-834': ['excessive iteration', 'inefficiency', 'performance degradation'],
                                    'CWE-770': ['unbounded resource allocation', 'resource allocation flaw', 'uncontrolled allocation'],
                                    'CWE-295': ['improper certificate validation', 'certificate verification', 'SSL/TLS weakness'],
                                    'CWE-74': ['code injection', 'injection vulnerability', 'script injection'],
                                    'CWE-310': ['cryptographic issue', 'insecure cryptography', 'weak encryption'],
                                    'Remain': ['general security issue', 'other threat', 'unspecified vulnerability']
})
